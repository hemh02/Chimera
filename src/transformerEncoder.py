...
class EncoderLayer(nn.Module):
    ...
    def forward(self, x, mask):
        ...
        # att = self.self_attn.attn
        # print(att.shape)
        # torch.set_printoptions(precision=4,sci_mode=False)
        # print(torch.mean(att, dim = 1)[0])
        # print(torch.sum(torch.sum(att, dim = 1), dim = 1)[0])
...
class Transformer(nn.Module):
    ...
    def forward(self, src, src_mask):
        ...
        att = self.encoder.layers[0].self_attn.attn
        pred_score = torch.mean(torch.mean(att, dim = 1), dim = 1)
        return out, pred_score

def make_model(N=1, d_model=300, d_ff=1024, h=12, dropout=0.1):
    c = copy.deepcopy
